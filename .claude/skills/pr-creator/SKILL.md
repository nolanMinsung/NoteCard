...
4. **라벨 선택**
   - `gh label list`로 저장소의 기존 라벨을 확인. `--label`은 **이미 존재하는 라벨**만 받으며, 없는 라벨을 넘기면 PR 생성 실패.
   - PR 성격에 맞는 라벨을 선택. 일반적 매핑:
     - 새 기능(`feat`) → `enhancement`
     - 버그 수정(`fix`) → `bug`
     - 문서(`docs`) → `documentation`
     - 그 외에 `chore`, `refactor`, `ci`, `test` 등
   - 적합한 라벨이 없으면 라벨 없이 생성. 임의로 새 라벨을 만들지 않음.

5. **`gh pr create` 호출**
   - `--base main --head <current_branch> --title "..." --body "$(cat <<'EOF' ... EOF)"`. 선택한 라벨이 있으면 `--label <name>`을 추가.
   - 생성된 PR URL 출력으로 보고.
